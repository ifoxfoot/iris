...
```{r}
#read in the oil spill data, pipe straight into janitor::clean_names(), so that all column names are coder friendly
oil_spill <- read_sf(here("Oil_Spill_Incident_Tracking_%5Bds394%5D-shp")) %>% 
  clean_names()

#read in county shapefile, again cleaning the names. select the names of the counties, the spatial data will come along with it!
ca_counties <- read_sf(here("ca_counties"), layer = "CA_Counties_TIGER2016") %>% 
  clean_names() %>% 
  dplyr::select(name)

#note, check the projection using st_crs(). In this case the oil spill data and the counties data are in the same projection.
...
ggplot(data = oil_spill_counts) + 
  geom_sf(aes(fill = n), color = "black", size = 0.2) + scale_fill_gradientn(colors = c("white", "orchid","purple")) +
