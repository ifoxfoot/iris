...
ggplot(data = palmetto, aes(x = width, y = length, fill = species)) + 
  ...
  labs(x = "width (cm)", y = "length (cm)",
       caption = 
      "Figure 1: Width and length of Sabal etonia vs Serenoa repens palmettos in South Central Florida
(Data: Abrahamson) Length is defined as the widest area of the canopy and width is the widest area
of the canpopy perpendicular to the length. From this graph you can see that Sabal etonia has a
slightly larger mean length.")
...
ggplot(data = palmetto, aes(x = green_lvs, y = height, color = species)) + 
  ...
  labs(x = "Number of Green Leaves", y = "Height (cm)",
       caption = 
         "Figure 2. Differences in number of green leaves vs height in Sabal Etonia and Serenoa repens
(Data: Abrahamson) Serenoa repens tends to have a higher number of green leaves. It is difficult
to tell if there is a meaningful difference in height")
...
broom::tidy(palmetto_blr) %>%
  ...
  kbl(caption = 
        "Palmetto Binary Logistic Regression Model Outputs - Reference Level = Sabal etonia (Data: Abrahamson)") %>% 
  kable_classic()
